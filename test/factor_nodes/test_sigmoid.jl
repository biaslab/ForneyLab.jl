...
@testset "EPSigmoidRealGB" begin
    ...
    @test ruleEPSigmoidRealGB(Message(Univariate, Bernoulli, p=0.5), Message(Univariate, Gaussian, m=1.0, v=0.5)) == Message(Univariate, Gaussian, m=1.0, v=2.5e11)
end

@testset "EPSigmoidRealGC" begin
    @test EPSigmoidRealGC <: ExpectationPropagationRule{Sigmoid}
    @test outboundType(EPSigmoidRealGC) == Message{Gaussian}
    ...
    @test ruleEPSigmoidRealGC(Message(Univariate, Categorical, p=[1.0, 0.0]), Message(Univariate, Gaussian, m=1.0, v=0.5)) == Message(Univariate, Gaussian, xi=0.6723616582693994, w=0.3295003993960708)
    @test ruleEPSigmoidRealGC(Message(Univariate, Categorical, p=[0.8, 0.2]), Message(Univariate, Gaussian, m=1.0, v=0.5)) == Message(Univariate, Gaussian, xi=0.4270174959448596, w=0.19914199922339604)
    @test ruleEPSigmoidRealGC(Message(Univariate, Categorical, p=[0.5, 0.5]), Message(Univariate, Gaussian, m=1.0, v=0.5)) == Message(Univariate, Gaussian, m=1.0, v=2.5e11)
end
...
@testset "EPSigmoidRealGP" begin
    ...
    @test ruleEPSigmoidRealGP(Message(Univariate, PointMass, m=NaN), Message(Univariate, Gaussian, m=1.0, v=0.5)) == Message(Univariate, Gaussian, m=1.0, v=2.5e11)
end
