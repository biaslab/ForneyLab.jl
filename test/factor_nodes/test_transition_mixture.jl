...
@testset "SVBTransitionMixtureOutNCCDX" begin
    ...
    @test isApplicable(SVBTransitionMixtureOutNCCDX, [Nothing, Message{Categorical}, Message{Categorical}, Distribution, Distribution]) 

    @test ruleSVBTransitionMixtureOutNCCDX(nothing, Message(Univariate, Categorical, p=[0.1, 0.9]), Message(Univariate, Categorical, p=[0.3, 0.7]), Distribution(MatrixVariate, PointMass, m=[0.1 0.2; 0.9 0.8]), Distribution(MatrixVariate, PointMass, m=[0.3 0.4; 0.7 0.6])) == Message(Univariate, Categorical, p=[0.33000000000033997, 0.6699999999996599])
end
...
@testset "SVBTransitionMixtureIn1CNCDX" begin
    ...
    @test isApplicable(SVBTransitionMixtureIn1CNCDX, [Message{Categorical}, Nothing, Message{Categorical}, Distribution, Distribution]) 
...
@testset "SVBTransitionMixtureZCCNDX" begin
    ...
    @test isApplicable(SVBTransitionMixtureZCCNDX, [Message{Categorical}, Message{Categorical}, Nothing, Distribution, Distribution]) 

    @test ruleSVBTransitionMixtureZCCNDX(Message(Univariate, Categorical, p=[0.2, 0.8]), Message(Univariate, Categorical, p=[0.1, 0.9]), nothing, Distribution(MatrixVariate, PointMass, m=[0.1 0.2; 0.9 0.8]), Distribution(MatrixVariate, PointMass, m=[0.3 0.4; 0.7 0.6])) == Message(Univariate, Categorical, p=[0.5479233226837061, 0.4520766773162939])
end
...
@testset "SVBTransitionMixtureA" begin
    ...
    @test isApplicable(SVBTransitionMixtureA, [Distribution, Nothing, Distribution]) 

    @test ruleSVBTransitionMixtureA(Distribution(Multivariate, Contingency, p=[0.3*[0.1 0.6; 0.2 0.1], 0.7*[0.1 0.6; 0.2 0.1]]), nothing, Distribution(MatrixVariate, PointMass, m=[0.3 0.4; 0.7 0.6])) == Message(MatrixVariate, Dirichlet, a=[1.03 1.18; 1.06 1.03])
end

@testset "MTransitionMixtureCCCDX" begin
    @test MTransitionMixtureCCCDX <: MarginalRule{TransitionMixture}
    @test isApplicable(MTransitionMixtureCCCDX, [Message{Categorical}, Message{Categorical}, Message{Categorical}, Distribution, Distribution]) 

    @test ruleMTransitionMixtureCCCDX(Message(Univariate, Categorical, p=[0.2, 0.8]), Message(Univariate, Categorical, p=[0.1, 0.9]), Message(Univariate, Categorical, p=[0.3, 0.7]), Distribution(MatrixVariate, PointMass, m=[0.1 0.2; 0.9 0.8]), Distribution(MatrixVariate, PointMass, m=[0.3 0.4; 0.7 0.6])) == Distribution(Multivariate, Contingency, p=[[0.0009966777408637877 0.017940199335548173; 0.035880398671096346 0.28704318936877077], [0.00697674418604651 0.08372093023255814; 0.06511627906976744 0.5023255813953489]])
end
