...
@testset "VBTransitionOut" begin
    ...
    @test isApplicable(VBTransitionOut, [Nothing, Distribution, Distribution]) 

    @test ruleVBTransitionOut(nothing, Distribution(Univariate, Categorical, p=[0.1, 0.4, 0.5]), Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == Message(Univariate, Categorical, p=[0.2994385327821292, 0.3260399327754116, 0.37452153444245917])
end
...
@testset "VBTransitionIn1" begin
    ...
    @test isApplicable(VBTransitionIn1, [Distribution, Nothing, Distribution]) 

    @test ruleVBTransitionIn1(Distribution(Univariate, Categorical, p=[0.1, 0.4, 0.5]), nothing, Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == Message(Univariate, Categorical, p=[0.208905960977562, 0.4255474215521219, 0.365546617470316])
end
...
@testset "VBTransitionA" begin
    ...
    @test isApplicable(VBTransitionA, [Distribution, Distribution, Nothing]) 

    @test ruleVBTransitionA(Distribution(Univariate, Categorical, p=[0.2, 0.7, 0.1]), Distribution(Univariate, Categorical, p=[0.1, 0.4, 0.5]), nothing) == Message(MatrixVariate, Dirichlet, a=[1.02 1.08 1.1; 1.07 1.28 1.35; 1.01 1.04 1.05])
end
...
@testset "SVBTransitionOutVCD" begin
    ...
    @test isApplicable(SVBTransitionOutVCD, [Nothing, Message{Categorical}, Distribution]) 

    @test ruleSVBTransitionOutVCD(nothing, Message(Univariate, Categorical, p=[0.1, 0.4, 0.5]), Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == Message(Univariate, Categorical, p=[0.3660714285714286, 0.27678571428571425, 0.35714285714285715])
end
...
@testset "SVBTransitionIn1CVD" begin
    ...
    @test isApplicable(SVBTransitionIn1CVD, [Message{Categorical}, Nothing, Distribution]) 

    @test ruleSVBTransitionIn1CVD(Message(Univariate, Categorical, p=[0.1, 0.4, 0.5]), nothing, Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == Message(Univariate, Categorical, p=[0.23000000000000007, 0.42999999999999994, 0.33999999999999997])
end
...
@testset "SVBTransitionADV" begin
    ...
    @test isApplicable(SVBTransitionADV, [Distribution, Nothing]) 

    @test ruleSVBTransitionADV(Distribution(Multivariate, Contingency, p=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3]), nothing) == Message(MatrixVariate, Dirichlet, a=[1.2 1.1 1.7; 1.4 1.3 1.3; 1.1 1.6 1.3])
end

@testset "MTransitionCCD" begin
    @test MTransitionCCD <: MarginalRule{Transition}
    @test isApplicable(MTransitionCCD, [Message{Categorical}, Message{Categorical}, Distribution]) 
    @test !isApplicable(MTransitionCCD, [Message{Categorical}, Message{Categorical}, Nothing]) 

    @test ruleMTransitionCCD(Message(Univariate, Categorical, p=[0.2, 0.7, 0.1]), Message(Univariate, Categorical, p=[0.1, 0.4, 0.5]), Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == Distribution(Multivariate, Contingency, p=[0.011799410029498528 0.023598820058997057 0.20648967551622419; 0.08259587020648967 0.247787610619469 0.3097345132743362; 0.002949852507374632 0.07079646017699116 0.04424778761061947])
end
...
@testset "MTransitionCCN" begin
    ...
    @test !isApplicable(MTransitionCCN, [Message{Categorical}, Message{Categorical}, Distribution]) 
...
@testset "averageEnergy" begin
    @test averageEnergy(Transition, Distribution(Univariate, Categorical, p=[0.2, 0.7, 0.1]), Distribution(Univariate, Categorical, p=[0.1, 0.4, 0.5]), Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == 1.1783637941354863
    @test averageEnergy(Transition, Distribution(Multivariate, Contingency, p=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3]), Distribution(MatrixVariate, PointMass, m=[0.2 0.1 0.7; 0.4 0.3 0.3; 0.1 0.6 0.3])) == 2.7886642527453405
end
