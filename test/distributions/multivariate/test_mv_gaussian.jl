...
facts("MvGaussian unit tests") do
    context("MvGaussian() should initialize a Gaussian distribution") do
        ...
        @fact logpdf(MvGaussian(m=ones(2), W=eye(2)), [0.0;0.0]) --> roughly(log(0.05854983152431917), atol=1e-8)
        @fact logpdf(MvGaussian(xi=ones(2), V=eye(2)), [0.0;0.0]) --> roughly(log(0.05854983152431917), atol=1e-8)
    end
