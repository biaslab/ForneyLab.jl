...
#         @fact y_subgraph.internal_schedule --> [ForneyLab.ScheduleEntry{SumProductRule}(edge(:q_y1).head), ForneyLab.ScheduleEntry{VariationalRule}(edge(:q_y1).tail)] # Include outgoing interface
#         @fact m_gam_subgraph.internal_schedule --> ForneyLab.convert(Schedule, [n(:m0).i[:out], n(:mN).i[:out], edge(:q_m1).tail, n(:gam0).i[:out], n(:gamN).i[:out], eg(:q_gam1).tail], SumProductRule) # Exclude outgoing interfaces
#     end
...
#         @fact f.factors[1].internal_schedule --> ForneyLab.convert(Schedule, [n(:t1).i[:out], n(:a1).i[:out], n(:t3).i[:out]], SumProductRule)
#         @fact f.factors[2].internal_schedule --> [ForneyLab.ScheduleEntry{SumProductRule}(n(:t2).i[:out]), ForneyLab.ScheduleEntry{VariationalRule}(n(:t2).i[:out].partner)]
