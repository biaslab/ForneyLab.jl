...
\begin{tikzpicture}
    ...
    \begin{scope}
        ...
        \node[] (gam_k_min_1) {$...$};
        \node[box, right of=gam_k_min_1, node distance=30mm] (gam_eq) {$=$};
        \node[right of=gam_eq] (gam_k) {$...$};

        \node[below of=gam_k_min_1, node distance=10mm] (m_k_min_1) {$...$};
        \node[box, right of=m_k_min_1] (m_eq) {$=$};
        \node[below of=gam_k, node distance=10mm] (m_k) {$...$};

        \node[box] (g) at (2.25, -2.0) {$\mathcal{N}$};        
        \node[clamped, below of = g] (y) {};

        \draw[line] (gam_eq) -- ($(gam_eq)+(0,-2.0)$) -> node[anchor=north]{$\gamma$} (g);
        \draw[line] (m_eq) -- ($(m_eq)+(0,-1.0)$) -> node[anchor=north]{$m$} (g);

        \path[line] (g) edge[->] node[anchor=west]{$y_k$} (y);

        \path[line] (gam_k_min_1) -> (gam_eq);
        \path[line] (gam_eq) -> (gam_k);
