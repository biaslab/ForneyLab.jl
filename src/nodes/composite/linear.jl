...
function updateNodeMessage!(outbound_interface_id::Int,
                            node::LinearCompositeNode,
                            inbound_messages_types::Type{Union(GaussianDistribution, GammaDistribution)})
    # Variational update function, takes the marginals as input.
    # Derivation for the update rule can be found in the derivations notebook.
...
function updateNodeMessage!(outbound_interface_id::Int,
    ...
    if typeof(node.noise_in.edge.marginal) == InverseGammaDistribution # Prior over variance given
    ...
    elseif typeof(node.noise_in.edge.marginal) == GammaDistribution # Prior over precision given
        ...
        error("Don't know whether to return gamma or inverse gamma message. Please preset the marginal on the noise_in interface's edge of LinearNode $(node.name).")
    end
