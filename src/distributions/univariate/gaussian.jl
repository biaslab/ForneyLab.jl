...
m(dist::Gaussian) = ensureParameter!(dist, Val{:m}).m # unsafe mean

S(dist::Gaussian) = ensureParameter!(dist, Val{:V}).V # unsafe covariance

v(dist::Gaussian) = S(dist) # unsafe variance
