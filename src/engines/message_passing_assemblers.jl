...
function assembleInitialization!(pf::PosteriorFactor)
    ...
    for entry in pf.schedule
        ...
        elseif isa(entry.interface.node, Union{Nonlinear, NonlinearGaussian}) && (entry.interface == entry.interface.node.interfaces[2]) && (entry.interface.node.g_inv == nothing)
