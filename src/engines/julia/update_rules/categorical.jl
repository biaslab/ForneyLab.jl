...
function ruleVBCategoricalOut(marg_out::Any, marg_p::Distribution{Multivariate})
    rho = clamp.(exp.(unsafeLogMean(marg_p)), tiny, Inf) # Softens the parameter
    
    Message(Univariate, Categorical, p=rho./sum(rho))
end

ruleVBCategoricalIn1(marg_out::Distribution, marg_p::Any) = Message(Multivariate, Dirichlet, a=unsafeMeanVector(marg_out) .+ 1.0)
